...
// コメント
{
  ...
  // ブロックコメント
  assertEquals(103, `100 + /* 20 + */ 3`); // 行の一部分をコメントアウトできる
  assertEquals(103, `100 + /*
    20 +
  */ 3`); // ブロックコメント内では改行してもよい

  // 入れ子コメントは + を5個まで増長できる
  assertEquals(103, `100 + /+     20 +                       +/ 3`);
  assertEquals(103, `100 + /++    20 +                      ++/ 3`);
  assertEquals(103, `100 + /+++++ 20 +                   +++++/ 3`);

  // 入れ子コメントは入れ子にできる
  assertEquals(103, `100 + /+++++ 20 +  /+++++ a +++++/  +++++/ 3`);

  // 入れ子コメントは対応する個数のより弱い終端には反応しない
  assertEquals(103, `100 + /++    20 +    +/ a /+       ++/ 3`);
  assertEquals(103, `100 + /+++   20 +    +/ a /+      +++/ 3`);
  assertEquals(103, `100 + /+++   20 +   ++/ a /++     +++/ 3`);
  assertEquals(103, `100 + /++++  20 +    +/ a /+     ++++/ 3`);
